% function Count()
    % combined SingleImageToImgEachDot and PredictUsingModel
    % with 'load net' command connecting the 2
    ...
    storeInPredictionFolder = true; %if false, create and store in folder with same name as img file
    browseImgDirectory = true; %if false, need to specity file name below
    ...
    %if centers_all is empty, show image output and return function
    ...
    %         fullDirectory = [pwd '/' input_img '_imgEachDot'];
            fullDirectory = [input_img '_imgEachDot'];
        end
        mkdir(fullDirectory);
    ...
    label_str = ['Signal count = ' num2str(numSignals)];
    position = [1 1 1 1];
    A_crop_annotated = insertObjectAnnotation(A_crop,'rectangle',position,label_str,...
        'TextBoxOpacity',0.5,'FontSize',18,'Color','cyan');

    figure(1)
    imshow(A_crop_annotated)
